...
fun UpcomingScreen(
    ...
    LaunchedEffect(Unit) {
        taskViewModel.fetchForUpcoming()
    }

    PullToRefreshBox(
        isRefreshing = state.isLoading,
        onRefresh = { taskViewModel.fetchForUpcoming() },
        modifier = Modifier.fillMaxSize()
    ) {
        if (!state.isLoading && state.upcomingByDay.isEmpty()) {
            Box(
                modifier = Modifier.fillMaxSize(),
            ...
            LazyColumn(
                ...
                state.upcomingByDay.forEach { (dateStr, tasks) ->
                    item(key = "header_$dateStr") {
                        Text(
                            text = DateHelpers.formatDueDate(dateStr),
                            style = MaterialTheme.typography.titleSmall,
                            fontWeight = FontWeight.Bold,
                            color = DateHelpers.dueDateColor(dateStr),
